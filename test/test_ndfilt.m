...
yf = ndfilt(y,filt='fillmiss',padval='symmetric',filtdim=1);
...
y(randi([1, numel(x)],50,1)) = nan;
y = cat(2, y, y + 2);
yf = ndfilt(y,filt='fillmiss',filtdim=1,padval='symmetric');
cellplot('plot',x,cat(2,y,yf),axpos=nan)
...
y = cat(2, y, y + 2, y + 3);
y = permute(y, [2, 3, 1]);
yf = ndfilt(y,filt='fillmiss',padval='symmetric',filtdim=3);
y = permute(y, [3, 1, 2]);
cellplot('plot',x,cat(2,y,yf),axpos=nan)
...
zf = ndfilt(z,filt='fillmiss',filtdim=[1,2],padval='symmetric');
cellplot('contourf',x,y,cat(3,z,zf),linestyle='none',axpos=nan)
...
z = cat(3, z, z + 1);
z = permute(z, [2, 3, 1]);
zf = ndfilt(z,filt='fillmiss',filtdim=[1,3]);
z = permute(z, [1, 3, 2]);
cellplot('contourf',x,y,cat(3,z,zf),linestyle='none',axpos=nan)
%% data 2D, filt 2D, fillmissn
[x,y] = meshgrid(linspace(0,1),linspace(1,2));
z = sin(10*x+20*y)+0.5*rand(size(x));
z(randi([1, numel(x)],200,1)) = nan;
zf = ndfilt(z,filt='fillmissn',filtdim=[1,2]);
cellplot('contourf',x,y,cat(3,z,zf),linestyle='none',axpos=nan)
%% data 3D, filt 2D, fillmissn
[x,y] = meshgrid(linspace(0,1),linspace(1,2));
z = sin(10*x+20*y)+0.5*rand(size(x));
z(randi([1, numel(x)],200,1)) = nan;
z = cat(3, z, z + 1);
...
z = cat(3, z, z + 1);
z = permute(z, [2, 3, 1]);
zf = ndfilt(z,filt='fillmissn',filtdim=[1,3]);
z = permute(z, [1, 3, 2]);
cellplot('contourf',x,y,cat(3,z,zf),linestyle='none',axpos=nan)
%% data 2D, filt 2D, interpn
[x,y] = meshgrid(linspace(0,1),linspace(1,2));
z = sin(10*x+20*y)+0.5*rand(size(x));
z(randi([1, numel(x)],200,1)) = nan;
zf = ndfilt(z,filt='interpn',filtdim=[1,2],filtker=[100,100]);
cellplot('contourf',x,y,cat(3,z,zf),linestyle='none',axpos=nan)